...
interface GetAllProjectOptions {
    ...
    title?: string;      // OPCIONAL: ¿Filtrar por título? (undefined = no filtrar)
    repoUrl?: string;      // OPCIONAL: ¿Filtrar por URL del repositorio? (undefined = no filtrar)
}
...
interface ProjectPaginatedResponse {
    projects: IProject[];   // ARRAY: Lista de projects (cada elemento es de tipo IProject)
    total: number;        // PRIMITIVO: Total de projects en la BD que cumplen filtros
...
// const { projects, total, page, totalPages } = await getAllProjects() ← Autocompletado perfecto
...
// ¿Por qué Promise<ProjectPaginatedResponse>? Porque es async y devuelve una estructura específica
export const getAllProjects = async (
    ...
    try {
        ...
        if (title !== undefined) {
            filter.title = title;  // Agrega: { title: "Proyecto 1" }
        }

        if (repoUrl) {
            filter.repoUrl = repoUrl;            // Agrega: { repoUrl: "https://github.com/user/repo" }
        }
        ...
        return {
            projects,        // IProject[] ✅ Cumple con la interface
