...
/**
 * deleteEducation - Función para eliminar un post de educación en la base de datos.
 *
 * @param educationData - Objeto con las propiedades necesarias para eliminar un EducationPost (title, content, etc.).
 * @returns Promise<IEducationPost> - Retorna una promesa que resuelve con una education eliminada.
 *
 *  Explicación:
 * 1. Se define la interfaz EducationInput para tipar claramente la estructura de entrada esperada (solo ID).
 ...
 * 3. Se elimina la educación directamente usando `Education.findByIdAndDelete()`.
 * 4. Si no se encuentra una educación con ese ID, se lanza un error personalizado `NotFoundError`.
...
export const addEducation = async (educationData: EducationInput): Promise<IEducation> => {
    ...
    try {
        ...
        const deletedEducation = await Education.findByIdAndDelete(validatedId);

        if (!deletedEducation) {
            throw new NotFoundError(
                "No se encontró una educación con ese ID", // frontend
                { id: validatedId },
                "Educación no encontrada para eliminación" // consola
            );
        }


        console.log(`Educación eliminada exitosamente: ${deletedEducation._id} - "${deletedEducation.degree}"`);
